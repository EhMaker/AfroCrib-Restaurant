...
class ShoppingCart {
  ...
  updateCartCounter() {
    const cartIcons = document.querySelectorAll(
      '#lg-bag a, #mobile a[href="cart.html"], .mobile-cart',
    );
    ...
    cartIcons.forEach((icon) => {
      ...
      if (itemCount > 0) {
        ...
        counter.className = icon.classList.contains("mobile-cart")
          ? "cart-badge"
          : "cart-counter";
    ...
    const itemCountElements = document.querySelectorAll(
      '[aria-label*="items in cart"], [aria-label*="item in cart"]',
    );
  ...
  showNotification(message, type = "info") {
    ...
    notification
      .querySelector(".cart-notification__close")
  ...
  getCartSummary() {
    return {
      ...
      formattedTotal:
        "₦" +
        this.getCartTotal().toLocaleString("en-NG", {
  ...
  createCartItemElement(item, index) {
    ...
    itemElement.innerHTML = `
            <div class="item-image">
                <img src="${item.image || (window.SITE_IMAGES || "assets/images/") + "pngegg.png"}" 
                     alt="${item.name}"
                     loading="lazy">
            </div>
  ...
  renderEmptyCart(container) {
    container.innerHTML = `
            <div class="empty-cart-state">
                ...
                <a href="${(window.SITE_PAGES || "") + "shop.html"}" class="btn-primary">
  ...
  updateOrderSummary() {
    const subtotalElement = document.querySelector(
      '.summary-row .value[aria-live="polite"]',
    );
  ...
  extractSingleProductData() {
    ...
    const description = descriptionSpan
      ? descriptionSpan.textContent.trim().substring(0, 100) + "..."
      : name;
  ...
  attachEventListeners() {
    ...
    document.addEventListener("click", (e) => {
      const cartIcon = e.target.closest(
        'a[aria-label*="cart"], .cart, i.fal.fa-shopping-cart',
      );
    ...
    if (
      addToCartButton &&
      addToCartButton.textContent.includes("Add To Cart")
    ) {
      addToCartButton.addEventListener("click", (e) => {
        ...
        if (productData) {
          ...
          if (productData.quantity > 1) {
            setTimeout(() => {
              this.showNotification(
                `Added ${productData.quantity} items of ${productData.name} to cart!`,
                "success",
              );
            }, 500);
          }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  console.log(
    "💡 Use debugCart() to see cart contents or clearCart() to empty cart",
  );
});
...
function getRelatedProducts(mainProduct, allProducts) {
  ...
  const otherProducts = allProducts.filter(
    (p) => p.mainImage !== mainProduct.mainImage,
  );
...
function saveProductDetails(productElement) {
  try {
    ...
    allProductsData = Array.from(allProductElements).map((el) =>
  ...
  allProducts.forEach((product) => {
    ...
    if (product.getAttribute("onclick")) {
      ...
      product.addEventListener("click", function (e) {
        ...
        saveProductDetails(this);
        window.location.href = (window.SITE_PAGES || "") + "Sproduct.html";
      });
    }
  });
});
