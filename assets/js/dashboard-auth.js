...
window.SUPABASE_URL =
  window.SUPABASE_URL || "https://fppnpevkegctkefjipoe.supabase.co";
window.SUPABASE_ANON_KEY =
  window.SUPABASE_ANON_KEY ||
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZwcG5wZXZrZWdjdGtlZmppcG9lIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjAwMzA4NTMsImV4cCI6MjA3NTYwNjg1M30.l183uTySyCDqDDPjqQd8zBcVNoLI-xuQ_WYSj8g4Dzw";
...
if (!window.supabaseClient) {
  if (window.supabase && typeof window.supabase.createClient === "function") {
    ...
    console.error(
      "⚠️ Supabase SDK not loaded. Make sure to include the Supabase script before dashboard-auth.js",
    );
  }
}
...
class DashboardAuth {
  ...
  async handleUserLogin() {
    ...
    try {
      ...
      if (session) {
        ...
        window.location.href = (window.SITE_PAGES || "") + "dashboard.html";
      } else {
  ...
  handleAdminLogin() {
    ...
    window.location.href = (window.SITE_PAGES || "") + "admin.html";
  }
  ...
  attachModalEventListeners(modal, type) {
    ...
    const toggleRegisterPassword = modal.querySelector(
      "#toggle-register-password",
    );
    if (toggleRegisterPassword) {
      toggleRegisterPassword.addEventListener("click", () => {
        const passwordInput = modal.querySelector(
          "#dashboard-register-password",
        );
  ...
  async handleLoginSubmit(modal, type) {
    ...
    const submitBtn = modal.querySelector(
      '#dashboard-auth-login-form button[type="submit"]',
    );
  ...
  async handleRegisterSubmit(modal) {
    ...
    const submitBtn = modal.querySelector(
      '#dashboard-auth-register-form button[type="submit"]',
    );
    this.setButtonLoading(submitBtn, true);
    ...
    try {
      ...
      // Show OTP step
      const emailDisplay = modal.querySelector(
        "#dashboard-verify-email-display",
      );
  ...
  async handleOTPSubmit(modal, type) {
    ...
    const submitBtn = modal.querySelector(
      '#dashboard-auth-otp-form button[type="submit"]',
    );
    this.setButtonLoading(submitBtn, true);
