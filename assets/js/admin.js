...
class AdminPanel {
  ...
  bindEvents() {
    ...
    if (imageUrlOption && imageFileOption) {
      imageUrlOption.addEventListener("change", () =>
        this.toggleImageInput("url"),
      );
      imageFileOption.addEventListener("change", () =>
        this.toggleImageInput("file"),
      );
    }
    ...
    if (fileInputGroup) {
      ...
      fileInputGroup.addEventListener("dragleave", (e) =>
  ...
  handleLogout() {
    ...
    setTimeout(() => {
      window.location.href = (window.SITE_ROOT || "") + "index.html";
    }, 500);
  }
  ...
  handleImageFile(event) {
    ...
    if (!allowedTypes.includes(file.type)) {
      this.showMessage(
        "Please select a valid image file (PNG, JPEG, JPG, GIF, WebP)",
        "error",
      );
      event.target.value = "";
      preview.innerHTML = "";
  ...
  handleAddFeature(e) {
    ...
    this.updateStats();
    this.showMessage(
      `Feature "${feature.name}" added successfully!`,
      "success",
    );
  }
  ...
  addRecipeToHomepage(recipe) {
    ...
    let homepageRecipes =
      JSON.parse(localStorage.getItem("homepageRecipes")) || [];
  ...
  addFeatureToHomepage(feature) {
    ...
    let homepageFeatures =
      JSON.parse(localStorage.getItem("homepageFeatures")) || [];
  ...
  renderRecipeList() {
    ...
    if (this.recipes.length === 0) {
      recipeList.innerHTML =
        '<p class="text-center">No recipes added yet. Add your first recipe using the form above!</p>';
      return;
    }

    recipeList.innerHTML = this.recipes
      .map(
        (recipe) => `
            <div class="recipe-item">
                <div class="recipe-info">
                    <img src="${recipe.image}" alt="${recipe.name}" class="recipe-image" onerror="this.src=(window.SITE_IMAGES||'assets/images/')+'cake1.png'">
  ...
  renderFeatureList() {
    ...
    if (this.features.length === 0) {
      featureList.innerHTML =
        '<p class="text-center">No custom features added yet.</p>';
      return;
    }

    featureList.innerHTML = this.features
      .map(
        (feature) => `
            <div class="feature-item">
                <div class="feature-info">
                    <img src="${feature.image}" alt="${feature.name}" class="feature-image" onerror="this.src=(window.SITE_IMAGES||'assets/images/')+'cake1.png'">
  ...
  renderBlogList() {
    ...
    if (this.blogs.length === 0) {
      blogList.innerHTML =
        '<p class="text-center">No blog posts added yet. Add your first blog using the form above!</p>';
      return;
    }

    blogList.innerHTML = this.blogs
      .map((blog, index) => {
  ...
  deleteFeature(id) {
    if (confirm("Are you sure you want to delete this feature?")) {
      ...
      let homepageFeatures =
        JSON.parse(localStorage.getItem("homepageFeatures")) || [];
      homepageFeatures = homepageFeatures.filter(
        (feature) => feature.id !== id,
      );
      localStorage.setItem(
        "homepageFeatures",
        JSON.stringify(homepageFeatures),
      );
  ...
  static addRecipeToMenu(recipe) {
    ...
    recipeElement.innerHTML = `
            <img src="${recipe.image}" height="210px" alt="${recipe.name}" onerror="this.src=(window.SITE_IMAGES||'assets/images/')+'cake1.png'">
  ...
  static addFeatureToPage(feature) {
    ...
    featureElement.innerHTML = `
            <img src="${feature.image}" height="115px" alt="${feature.name}" onerror="this.src=(window.SITE_IMAGES||'assets/images/')+'cake1.png'">
            <h6>${feature.name}</h6>
        `;
